...
class UserManager:
    ...
    async def login(user_data):
        user_do = await database.fetch_one(user.select().where(user.c.email_id == user_data["email_id"]))
        if not user_do:
    ...
    @staticmethod
    async def get_user_by_email(email_id):
        return await database.fetch_all(user.select().where(user.c.email_id == email_id))
